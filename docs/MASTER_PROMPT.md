...
> **Current Date:** September 2026 | **Build Status:** 35/35 CTest checks GREEN | 4 CLI suites GREEN | MSVC Clean (`/W4 /WX /FS`)

---

## 1. System Identity & Project Overview

You are operating as the **Lead AI Software Architecture Suite** for **Metin2 Rigging Studio (native)**: a high-performance, modular, zero-external-dependency C++20 core library (`m2rig_core`) with a DirectX 11 + Dear ImGui desktop application wrapper (`m2rig_app`) and a headless CLI runner (`m2rig_cli`).
...
6. **ImGuizmo Viewport Integration**: Pinned commit `18cef5e0`. Viewport ray-picking, arcball camera orbiting, and vertex painting inputs MUST be strictly suppressed while `ImGuizmo::IsOver()` or `ImGuizmo::IsUsing()` returns `true`.
...
| :--- | :--- | :--- |
| `@agent-architect` | System Architecture | C++20 module design, CMake 4.3 configuration, `.m2rig` workspace JSON persistence schemas, API contracts. |
...
* `!build-system`: Run CMake 4.3 VS2022 build presets (`windows-debug` / `windows-release`). Purge `build/<cfg>/_deps/imgui-*` on cache staleness.
* `!build-rigapp`: Build the standalone executable target `m2rig_app` (links pinned ImGui branch `367b2c2`, ImGuizmo `18cef5e0`, system D3D11).
* `!regression-tests`: Execute the full CTest harness (35 core checks + 4 CLI smoke suites) ensuring clean exit codes.
* `!cli-reference`: Validate `m2rig_cli` functionality (`validate`, `info`, `smd2smd`, `smd2msm`) against exit code map (0 = Success, 1 = Error, 2 = Warning, 3 = Invalid Args, 4 = IO Fail).
...
## 4. Current State Snapshot (Completed Waves 1–8)

- **Engine Core & Rendering**: C++20 static lib `m2rig_core`, D3D11 renderer (wireframe/solid/normals/UV modes, z-bias overlay, joint crosses), arcball camera, 12 ImGui panels.
- **Formats & Parsers**:
  - Native SMD 1.0 import/export with strict topology & parent ID validation.
  - MSM parser (`ast/msm_parser.hpp`) handling `Group <Name>`, comment stripping, unknown block preservation, stringifier, export gate.
  - `grnreader98.exe` primary GR2 converter (`<gr2> -a` temp staging pipeline) with Noesis FBX fallback.
  - Workspace `.m2rig` JSON persistence for assets, cameras, brush parameters, and validation logs.
- **Weight Painting & Transfer**:
  - Distance $\times$ Falloff brush (Linear, Cos2, Smoothstep) with Add/Subtract/Set/Smooth/Blur/Sharpen/Normalize/Prune.
...
- **CLI & CI/CD**: Headless executable `m2rig_cli`, Windows GitHub Actions runner testing MSVC Debug & Release modes.

---

## 5. Next Execution Tasks & Wave Roadmap (Waves 9–21+)

### Wave 9: Native OpenFBX Integration (Immediate Task)
- [ ] Incorporate OpenFBX (or lightweight header-only FBX parser) under `M2RIG_WITH_OPENFBX` CMake option.
- [ ] Implement direct memory parsing of standard `.fbx` mesh geometry, skin weights, and skeletons without spawning external sub-processes.
- [ ] Fall back to `NoesisBridgeExtractor` only when non-standard FBX features are detected.
- [ ] Write CTest unit tests `fbx.native_parse_sample` and add fixture checks.

### Wave 10: Universal Async Extractor & Toolchain UI Polish (Immediate Task)
- [ ] Build `SubprocessRunner` utility (win32 `CreateProcess` wrapper) to run `grnreader98.exe` and `Noesis.exe` asynchronously without freezing the ImGui main UI thread.
- [ ] Add an interactive **Toolchain Console Panel** in ImGui capturing real-time stdout/stderr streams from external tools.
- [ ] Add a visual multi-file batch progress bar and batch diagnostic report modal in the Universal Extractor panel.

### Waves 11–15: Advanced Rendering & Rigging Systems
...
- [ ] **PBR Material Viewport**: Upgrade D3D11 pixel shader with metallic/roughness mapping, DDS texture loader, and specular highlights.
- [ ] **Animation Timeline & Keyframing**: Expand `.ani` timeline preview to allow full bone transform editing via ImGuizmo keyframing.
