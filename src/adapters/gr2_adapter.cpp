...
Gr2ExportStatus exportGr2ViaBridge(
    ...
    if (config.keepTempSmd) {
        ...
        tempSmd = std::filesystem::temp_directory_path() / 
                  (outputPath.stem().string() + "_temp.smd");
    }
...
Result<std::string> convertGr2ToSmdViaGrnReader(const std::filesystem::path& gr2Path,
    ...
    const std::filesystem::path staged = stageDir / gr2Path.filename();
    ...
    std::wstring cmd = L"\"" + exe.wstring() + L"\" \"" + staged.wstring() + L"\" -a";
    ...
    std::error_code rm;
    std::filesystem::remove(outSmd, rm);
